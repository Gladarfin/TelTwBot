...
func (d *Database) getOrCreateUser(ctx context.Context, username string) (int, error) {
	...
	err := d.WithTransaction(ctx, func(tx *sql.Tx) error {
		...
		`
		err := tx.QueryRowContext(ctx, userIdQuery, username).Scan(&userID)
		if err == nil {
			return nil
		}
		...
		const addUserQuery = `
			...
			RETURNING id
		`
		return tx.QueryRowContext(ctx, addUserQuery, username).Scan(&userID)
	})
...
func (d *Database) getExistingUserStats(ctx context.Context, tx *sql.Tx, userID int) ([]UserStats, error) {
	const query = `
...
func (d *Database) createDefaultStatsForUser(ctx context.Context, userID int) error {
	return d.WithTransaction(ctx, func(tx *sql.Tx) error {
		statTypes, err := d.getAllStatTypes(ctx, tx)
		if err != nil {
		...
		for _, statType := range statTypes {
			_, err := tx.ExecContext(ctx, insertMissingStats, userID, statType)
			if err != nil {
...
func (d *Database) UpdateUserStats(ctx context.Context, userID int, statName string, value int) error {
	...
	err := d.WithTransaction(ctx, func(tx *sql.Tx) error {
		...
		newValue := min(max(curValue+value, stat.MinValue), stat.MaxValue)
...
func (d *Database) getAllStatTypes(ctx context.Context, tx *sql.Tx) ([]string, error) {
...
func (d *Database) UpdateUserStat(ctx context.Context, username string, stat string, val int) (string, int, error) {
	...
	err := d.WithTransaction(ctx, func(tx *sql.Tx) error {
		...
		err = tx.QueryRowContext(ctx, `
			...
			FROM stat_types
			WHERE name = $1`, stat).
			Scan(&statTypeID, &statName, &minValue, &maxValue)

		if err != nil {
			if err == sql.ErrNoRows {
				return fmt.Errorf("stat %s not found", stat)
			}
			return fmt.Errorf("failed to get stat type: %w", err)
		}
		...
		err = tx.QueryRowContext(ctx, getValueQuery, userID, statTypeID).Scan(&curStatValue)
		if err != nil {
			return fmt.Errorf("failed to get current stat value: %w", err)
		}
		...
		if err != nil {
			return fmt.Errorf("failed to get free-points stat id: %w", err)
		}

		err = tx.QueryRowContext(ctx, getValueQuery, userID, freePointsStatID).Scan(&curFreePoints)

		if err != nil {
			if err == sql.ErrNoRows {
				curFreePoints = 0
			} else {
				return fmt.Errorf("failed to get free points: %w", err)
			}
		}

		if curFreePoints < val {
			return fmt.Errorf("not enough free points: need %d, have %d", val, curFreePoints)
		}

		newStatValue = min(curStatValue+val, maxValue)
